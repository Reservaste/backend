...
-- ============================================================
-- 5. Lo que NO va en esta migración, y por qué
-- ============================================================
-- El mismo barrido encontró dos casos más de policy `FOR ALL` con DELETE
-- incluido -- `organization_roles` y `service_plan_services` -- que también
-- cierra ADR-0037. Los dos viven en
-- `20260923240000_phase36b_role_and_plan_scope_delete_closed.sql`, no acá,
-- porque los dos dependen de objetos que crea la Fase 32
-- (`20260923190000_phase32_configurable_roles.sql`):
--
--   * `organization_roles` es una tabla de la Fase 32. Un
--     `drop policy ... on public.organization_roles` acá hace fallar la
--     migración entera con `relation "public.organization_roles" does not
--     exist` en cualquier base que se construya desde cero sin la Fase 32
--     -- exactamente lo que hace CI.
--   * `service_plan_services` sí existe desde la Fase 22, pero la policy
--     que hay que reemplazar, `service_plan_services_write_owner`, la crea
--     la Fase 32 (la Fase 22 la había llamado
--     `service_plan_services_write_staff`, con `is_organization_member`).
--     Acá el `drop policy if exists` no daría error: sería peor -- un no-op
--     silencioso que deja viva la policy `FOR ALL` real y agrega dos
--     policies nuevas encima, con el DELETE todavía abierto y el test en
--     verde por el motivo equivocado.
--
-- Las secciones 1-4 no dependen de nada posterior a la Fase 13: las dos
-- vistas son de la Fase 4 (ADR-0008) y `organization_members_write_owner`
-- es de la Fase 1. Esta migración se aplica sola, en orden, sobre el
